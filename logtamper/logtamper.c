...
/*Help info*/
void usage(void){
	printf("Logtamper v 1.11 for linux, %d bits.\n", sizeof(unsigned int) * 8);
	printf("Copyright (C) 2018 by leonstill <leonstill@gmail.com>\n");
	printf("Usage:\n");
	printf("  logtamper [-f utmp_filename] -h username hostname     Hide username connected from hostname\n");
	printf("            [-f wtmp_filename] -w username hostname     Erase username from hostname in wtmp file\n");
	printf("            [-f wtmp_filename] -l                       List login info (user,host) details. \n");
	printf("            [-f lastlog_filename] -m username hostname ttyname YYYY[:MM[:DD[:hh[:mm[:ss]]]]]  modify lastlog info\n");
}
...
int main(int argc,char **argv){
		...
		while((ch = getopt(argc,argv,"lf:hwm"))!= EOF){
...
int listLogInfo(const char *filename)
{
	int ret = 0;
	...
	while ((utp = getutxent()))
	{
		...
		char time_str_buff [100];
		if (utp && utp->ut_type == USER_PROCESS)
		{
			tm_now = localtime(&utp->ut_tv.tv_sec);
			//time_t tm;
			//time(&tm);
			//tm_now = localtime(&tm);
			snprintf(time_str_buff, sizeof(time_str_buff), "%04d-%02d-%02dT%02d:%02d:%02d", tm_now->tm_year + 1900, tm_now->tm_mon+1, tm_now->tm_mday, tm_now->tm_hour, tm_now->tm_min, tm_now->tm_sec);
			printf("[%02d] %s %s %s\n", index, time_str_buff, utp->ut_user, utp->ut_host);
			// printf("[%02d] %s %s \n", index, utp->ut_host, utp->ut_user);
			index++;
		}
